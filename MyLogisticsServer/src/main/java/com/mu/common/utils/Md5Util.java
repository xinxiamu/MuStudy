...
public class Md5Util {
	...
	public static void main(String[] args) {
		System.out.println(getMD5("a1234567"));
	}
	...
	public static String getMD5(String inbuf) {
		...
		for (i = 0, j = 0; j < 8; i++, j += 4) {
			bits[j] = (byte) (count[i] & 0xffL);
			bits[j + 1] = (byte) ((count[i] >>> 8) & 0xffL);
			bits[j + 2] = (byte) ((count[i] >>> 16) & 0xffL);
			bits[j + 3] = (byte) ((count[i] >>> 24) & 0xffL);
		}
		index = (int) (count[0] >>> 3) & 0x3f;
		...
		md5Update(bits, 8);
		int w, e;
		for (w = 0, e = 0; e < 16; w++, e += 4) {
			digest[e] = (byte) (state[w] & 0xffL);
			digest[e + 1] = (byte) ((state[w] >>> 8) & 0xffL);
			digest[e + 2] = (byte) ((state[w] >>> 16) & 0xffL);
			digest[e + 3] = (byte) ((state[w] >>> 24) & 0xffL);
		}
		str = "";
		for (int k = 0; k < 16; k++) {
			str += byteHEX(digest[k]);
		}
